...
def ventana_modificacion(id_objeto):
    ...
    ventana_modificacion.title("Modificar Deporte")
...
def ventana_ingreso_id():
    def obtener_id_y_modificar():
        ...
        if id_objeto:
            ...
            messagebox.showwarning("Advertencia", "Por favor, ingresa un ID de Deporte.")

    # Crear ventana para ingresar el ID
    ventana_ingreso = tk.Toplevel()
    ventana_ingreso.title("Buscar Deportemedad")
   # ventana_ingreso.attributes('-topmost', True)  # Mantener en primer plano
    ...
    tk.Label(ventana_ingreso, text="ID de la Deporte:").pack()
    id_entry = tk.Entry(ventana_ingreso)  # Definimos id_entry aquí
    id_entry.pack(pady=5)
    id_entry.focus_set()  # Establecer el foco en el campo de ID

    buscar_btn = tk.Button(ventana_ingreso, text="Modificar Deporte", command=obtener_id_y_modificar)
